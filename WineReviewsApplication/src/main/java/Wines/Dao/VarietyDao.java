...
public class VarietyDao {
	...
	public Variety create(Variety variety) throws SQLException {
		String insertVariety = "INSERT INTO Variety(Type, WineryTitle) VALUES(?,?);";
		...
		try {
			...
			insertStmt.setString(2, variety.getWineTitle());
	...
	public Variety getVarietyByType(String typeName) throws SQLException {
		...
		try {
			...
			if(results.next()) {
				...
				String resultWineTitle = results.getString("Type");
				Variety variety = new Variety(resultType, resultWineTitle);
				return variety;
			}
		} catch (SQLException e) {
			e.printStackTrace();
