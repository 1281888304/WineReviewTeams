...
public class ProfessionalReviewsDao {
	protected ConnectionManager connectionManager;
	 ...
	 public ProfessionalReviews create(ProfessionalReviews professionalReview) throws SQLException {
	  String insertProfessionalReview = "INSERT INTO ProfessionalReviews(ProfessionalReviewId, Created, Content, TasterName, WineTitle) VALUES(?,?,?,?,?);";
	 ...
	 public ProfessionalReviews getProfessionalReviewFromProfessionalReviewId(int professionalReviewId) throws SQLException {
	  String selectProfessionalReview = "SELECT ProfessionalReviewId, Created, Content, TasterName, WineTitle FROM ProfessionalReviews WHERE ProfessionalReviewId=?;";
	  Connection connection = null;
	  ...
	  try {
	   ...
	   if(results.next()) {
	    int professional_review_id = results.getInt(professionalReviewId);
	    Date created = new Date(results.getTimestamp("Created").getTime());
	    String content = results.getString("Content");
	    String taster_name = results.getString("TasterName");
	    String wine_title = results.getString("WineTitle");
	    ProfessionalReviews professionalReview = new ProfessionalReviews(professional_review_id,created, content, new Taster(taster_name), new Wine(wine_title));
	    return professionalReview;
	   }
	  } catch (SQLException e) {
	   e.printStackTrace();
	 ...
	 public ProfessionalReviews delete(ProfessionalReviews professionalReview)throws SQLException {
	  String deleteProfessionalReview = "DELETE FROM ProfessionalReviews WHERE ProfessionalReviewId=?;";
	  Connection connection = null;
